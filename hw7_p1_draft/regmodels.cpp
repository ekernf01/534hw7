...
void AddRegression(int nMaxRegs, LPRegression regressions,int lenA,int* A,double logmarglikA)
{
  ...
  //Count the regressions. Re-use these already-allocated temp variables.
  p = regressions;
  pnext = p->Next;
  int nReg = 0;
  while(NULL!=pnext)
  ...
  //Remove the worst regression if we have reached capacity.
  for(int j=0; j<nReg-nMaxRegs; j++){
    DeleteLastRegression(regressions);
    printf("deleted one regression\n");
    if(j >= 1){
      cout << "Something is wrong in AddRegression!" << endl;
      cout << "Should never delete more than one regression every time you add one!" << endl;
    }
  }
